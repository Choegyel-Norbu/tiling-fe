...
function Carousel({
  ...
  useEffect(() => {
      if(setApi) {
          setApi({
              scrollPrev: () => handleIndexChange(Math.max(0, currentIndex - 1)),
              scrollNext: () => handleIndexChange(currentIndex + 1), // Need logic to limit max index
              canScrollPrev: () => currentIndex > 0,
              canScrollNext: () => true, // Simplified for now
              on: (event, cb) => {}, // Mock implementation
              off: (event, cb) => {} // Mock implementation
          })
      }
  }, [setApi, currentIndex]);
...
function CarouselContent({
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      ...
      setVisibleItemsCount(visibleCount || 1); // Default to 1 to avoid division by zero
    }, options);
